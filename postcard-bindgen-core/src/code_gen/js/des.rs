...
pub fn gen_deserializer_code() -> Tokens {
    quote! {
        class Deserializer {
            ...
            get_int8 = () => new Int8Array([this.pop_next()])[0]
            ...
            deserialize_number = (n_bytes, signed) => { if (n_bytes === U8_BYTES) { return this.get_int8() } else if (n_bytes === U16_BYTES || n_bytes === U32_BYTES || n_bytes === U64_BYTES || n_bytes === U128_BYTES) { const val = this.try_take(n_bytes); return to_number_if_safe(signed ? de_zig_zag_signed(val) : val) } else { throw "byte count not supported" } }
